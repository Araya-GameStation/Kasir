...
let state={currentCategory:null,cart:[],isProcessing:false};
...
function renderCategories(){
 ...
 return db.categories.map(c=>`<button onclick="selectCategory(${c.id})">${c.name}</button>`).join("");
}
...
function renderHistory(){
 if(db.transactions.length===0){
  app.innerHTML="<h2>Belum ada transaksi</h2><button onclick='render()'>Kembali</button>";
  return;
 }
 app.innerHTML=`
 <h2>Riwayat</h2>
 <button onclick="clearHistory()">Hapus Semua</button>
 <hr>
 ${db.transactions.map(t=>`
  <div>${t.date} - ${formatRupiah(t.total)}</div>`).join("")}
 <hr><button onclick='render()'>Kembali</button>`;
}

function clearHistory(){
 showModal("Hapus semua riwayat?",()=>{
  db.transactions=[];saveDB();renderHistory();
 });
}
...
function renderSettings(){
 app.innerHTML=`
 <h2>Pengaturan</h2>
 <input id="footerInput" placeholder="Footer struk" value="${db.settings.footer}">
